...
{
Processor proc1; // Processor 1
Processor proc2; //Processor 2
Request * reqhead;  // Request list
Request * nextreq;  // Next arriving request.
Reqqueue  reqq1; //queue for processor 1
Reqqueue  reqq2; //queue for processor 2

int ctime;
int blocked=0; // Number of requests blocked

nextreq=list;
proc1.busy=0;   // Initialize processor
proc2.busy=0;	//Initialize second processor
reqq1.occ = 0;  // Initialize the request queue. 
...
for (ctime = 0; nextreq != NULL; ctime++) {
 ...
 {	 
   ...
   if (proc1.busy == 0 && reqq1.occ > 0) {
      proc1.depart = ctime + reqq1.service - 1;
      proc1.busy = 1;
      reqq1.occ = reqq1.occ - 1;
   } 
   ...
   // Transfer a request from request queue to idle processor 
   if (proc1.busy == 0 && reqq1.occ > 0) {
      proc1.depart = ctime + reqq1.service;
      proc1.busy=1;
      reqq1.occ--;
   } 
 }
